	...
	var Demo = function (_React$Component) {
	    ...
	    _createClass(Demo, [{
	        ...
	        value: function render() {
	            ...
	            var _photos$currPhoto = _slicedToArray(photos[currPhoto], 2);

	            var currWidth = _photos$currPhoto[0];
	            var currHeight = _photos$currPhoto[1];

	            var widths = photos.map(function (_ref2) {
	            ...
	            var leftStartCoords = widths.slice(0, currPhoto).reduce(function (sum, width) {
	                return sum - width;
	            }, 0);
	            var configs = [];
	            photos.reduce(function (prevLeft, orig, i) {
	                configs.push({
	                    left: react_motion_1.spring(prevLeft, springSettings),
	                    height: react_motion_1.spring(currHeight, springSettings),
	                    width: react_motion_1.spring(widths[i], springSettings)
	                });
	            ...
	            return React.createElement(
	                ...
	                React.createElement('input', { type: 'range', min: 0, max: photos.length - 1, value: currPhoto.toString(), onChange: this.handleChange.bind(this) }),
	                React.createElement(
	                    'div',
	                    { className: 'demo4' },
	                    React.createElement(
	                        react_motion_1.Motion,
	                        { style: { height: react_motion_1.spring(currHeight), width: react_motion_1.spring(currWidth) } },
	                        function (container) {
	                            return React.createElement(
	                                'div',
	                                { className: 'demo4-inner', style: container },
	                                configs.map(function (style, i) {
	                                    return React.createElement(
	                                        react_motion_1.Motion,
	                                        { key: i, style: style },
	                                        function (style) {
	                                            return React.createElement('img', { className: 'demo4-photo', src: './' + i + '.jpg', style: style });
	                                        }
	                                    );
	                                })
	                            );
